...
public class BoardManager : MonoBehaviour
{
    ...
    #region Get Starting possible ID
    ...
    private List<int> GetStartingPossibleID(int x, int y)
    {
        ...
        if(y > 1 && tiles[x, y - 1] == tiles[x, y - 2])
        {
    ...
    private IEnumerator RepositionTiles(System.Action onCompleted)
    {
        ...
        for(int x = 0; x < size.x; x++)
        {
            for(int y = 0; y < size.y; y++)
            {
                ...
                {
                    Debug.Log("skip reposition");
                    continue;
                }

                isCompleted.Add(false);

                //Debug.Log("Tile(" + x + "," + y + ") pos(" + tiles[x,y].transform.position + ")");
                //Debug.Log("target(" + x + "," + y + ") pos(" + targetPosition + ")");
